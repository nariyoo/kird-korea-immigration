...
side. The other thirteen hold the breakdowns those summaries aggregate.

| File | One row is | Years | Rows |
|---|---|---|---|
| `national_annual.csv` | a year | 2008-2025 | 18 |
| `summary_by_sido.csv` | a province and a year | 2006-2025 | 334 |
| `summary_by_sigungu.csv` | a district and a year | 2008-2025 | 4,490 |
| `summary_by_eupmyeondong.csv` | a sub-district and a year | 2014-2024 | 39,439 |
| `nationality_by_sigungu.csv` | a district, nationality and year | 2008-2025 | 189,185 |
| `visa_by_sigungu.csv` | a district, visa status and year | 2008-2025 | 80,902 |
| `visa_by_nationality.csv` | a population, nationality, visa and year | 2006-2025 | 250,301 |
| `age_sex_national.csv` | a nationality, age band, sex and year | 2009-2025 | 109,029 |
| `language_demand.csv` | a language, place and year | 2006-2025 | 110,318 |
| `ethnic_enclaves.csv` | an enclave district, nationality and year | 2008-2025 | 796 |
| `segregation_by_nationality.csv` | a nationality and year | 2014-2025 | 1,169 |
| `region_segregation.csv` | a world region of origin and year | 2014-2025 | 132 |
| `naturalization_annual.csv` | a year and processing route | 2011-2025 | 150 |
| `naturalization_by_country.csv` | a year, former nationality and route | 2009-2025 | 16,019 |
| `naturalization_by_age.csv` | a year, age band and route | 2009-2025 | 1,662 |
...
composition and `evenness` is Pielou's, `shannon_H / ln(n_nationalities)`. `continent_H` groups
...
`code/run_pipeline.py` is the complete step list in dependency order. Phase 1 reads the raw
yearbooks and writes the harmonized panel; phase 2 turns that into the released tables. It ends on
an integrity audit that must print `AUDIT CLEAN`. Each step is numbered for its position, `01`
through `20` for phase 1 and `21` through `33` for phase 2, and the unnumbered files are the three
shared modules and the scripts that run on demand.
...
python code/run_pipeline.py --from 27_export_dataset.py  # resume at a step
python code/sync_public_code.py --check            # has the published code drifted?
```

`code/kird_paths.py` resolves every path from the project root, so the scripts run from any location
and `KIRD_ROOT` points them at another checkout. Python 3.10 or newer. Every step is described in
[`code/README.md`](code/README.md).

The raw ministry workbooks are not in this repository, so a bare checkout documents the build rather
than reproducing it. To rebuild from scratch, download the yearbooks from the agencies in section 8
into `01_raw_data/` under the layout that `code/README.md` describes.
...
MOIS publishes on its own schedule, so the MOIS-sourced files stop at 2024 while the MOJ-sourced
ones run to 2025.
...
continent subtotals by 1,367. The figures are published as issued.
