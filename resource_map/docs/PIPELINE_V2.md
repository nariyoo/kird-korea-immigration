...
## 조용히 반영되지 않는 실패 (2026-08-20에 다섯 번 나왔다)

이 파이프라인이 실패하는 방식은 대체로 하나다. 단계가 성공하고, 수치가 그럴듯하고,
아무 줄도 반영되지 않았다는 것을 말하지 않는다. 하루에 다섯 번 나왔으므로 유형으로
적어 둔다.

**1. 죽은 단계를 지나친다.** 실행 스크립트가 `python ... | tail -24` 였다. 파이프의
종료코드는 `tail` 의 것이므로 `set -e` 가 만족되고, `code_inclusion.py` 가 NameError로
죽었는데도 하류가 전부 돌아 **직전 실행의** `inclusion_coded.csv` 로 빌드가 끝났다.
사람이 판정한 56건이 반영되지 않았고 census 는 95%를 찍었다. 지금은 단계가 죽으면
`STEP FAILED` 를 찍고 멈춘다.

**2. 실행 목록에서 단계가 빠진다.** `run_full.sh` 에 `build_frame` 이 없었다. 원장을
읽는 단계가 그것인데, 그동안 손으로 따로 돌렸기 때문에 티가 나지 않았다. 주소 판정
37건을 원장에 넣고 파이프라인을 돌렸더니 반영이 안 되어서 드러났다.

**3. 철회한 것이 철회되지 않는다.** 잘못된 명부를 `data/raw/v2/archive/` 로 옮겼는데
`load_rosters` 가 그 폴더도 읽었다. 말 주인 협회 두 곳이 공개본에 그대로 남았다.

**4. 원장이 채우기만 하고 고치지는 않는다.** 주소 원장은 "주소가 없는 행을 채우는"
용도로 만들어져 `if not road_address` 조건이 붙어 있었다. 나중에 같은 원장에 **두 공식
출처 중 어느 쪽이 현재인지 판정한 값**이 들어왔고, 기존 주소가 있으므로 37건이 전부
무시됐다. 이것은 US census 실패 #4(ledger 가 protect 만 하고 materialize 하지 않는다)와
같은 것이 반대 방향으로 나타난 경우다.

**5. 뒤 단계의 휴리스틱이 앞 단계의 판정을 덮는다.** 병합이 "가장 긴 주소를 남긴다"를
쓴다. 두 명부가 같은 주소를 다르게 적었을 때는 맞는 규칙이고, 판정 결과를 상대로는
정확히 틀린 규칙이다. `횡성로 379(횡성군종합보건복지타운 3층)` 가
`횡성로 393, 3층 (365채움관)` 보다 길고, 앞의 것이 센터가 떠난 주소다. 원장에서 온
주소는 표시를 달고 병합에서 무조건 이긴다.

**공통 처방**: 원장이나 판정을 적용하는 코드는 **적용한 수와 적용하지 못한 수를 함께
찍는다.** `manual addresses: 426 filled, 341 replaced, 228 in the ledger, 0 matched no
row` 처럼 네 수가 함께 나와야 한 수가 0인 것이 눈에 띈다.

## 브라우저는 사다리의 한 칸이지 실행 조건이 아니다

`fetchpage.py` 는 requests → curl_cffi → Playwright 로 올라간다. 드라이버가 중간에
사라지면 `Connection closed while reading from the driver` 가 나는데, 이것은 어느 한
페이지에 대한 사실이 아니라 실행 환경에 대한 사실이다. 이 예외가 위로 던져져 파이프라인
전체가 두 번 끝났다. 지금은 브라우저를 띄우지 못하거나 드라이버가 사라지면 그 칸을 끄고
아래 칸으로 계속한다. 사다리를 만든 이유가 그것이다.
