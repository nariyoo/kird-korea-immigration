"""
Build 2006-2024 foreign resident dashboard from KIS yearbook excel files.
...
# The 2015-2024 export uses the site's current layout (name+code in one column,
...
# listed last (later files win on duplicate keys); build_population_long
# then cuts every year past LAST_YEAR.
POPULATION_FILES = [
...
# 국적처리 (귀화 등): 연도별 추이 + 국가별 + 연령별.
# 4장의 국가별·연령별 표는 그 판의 해 하나만 담으므로 LAST_YEAR 판을 읽는다.
# 화면은 이 최신 단면을 쓰고, 배포본은 09 직전에 RELEASE_LAST_YEAR 로 잘린다.
...
# 원자료를 어디까지 읽는가. 화면이 이만큼 보여 준다. 배포본은 더 앞에서 끊고,
# 그 자리는 kird.cut_release_years() 한 곳이고 09 가 부른다(RELEASE_LAST_YEAR).
for _d in (STAY_FILES, REG_FILES, REGION_COUNTRY_FILES, AGE_FILES,
           SHORT_FILES, SOJOURN_FILES):
    for _y in [y for y in _d if isinstance(y, int) and y > LAST_YEAR]:
        del _d[_y]
...
# Sanity check: total residents in the latest year (2.65M in 2024)
...
def build_population_long():
    ...
    long = pd.concat(parts, ignore_index=True)
    # 인구 파일은 2025년까지 담고 있지만 자료 전체가 LAST_YEAR 에서 끝난다.
    long = long[long["year"] <= LAST_YEAR]
...
def compute_indices(region_df, pop_df):
    ...
    # 주민등록(total_pop)은 한국 국적자의 명부다. 외국인이 애초에 들어 있지
    # 않으므로 빼지 않는다. 09 의 분리지수도 같은 기준을 쓴다(k = resident_pop).
    merged["korean_est"] = merged["total_pop"].clip(lower=0)
    ...
    for yr in out["years"]:
        ...
        pop_idx = yr_pop.set_index(["sido", "sigungu"])[
            ["total_pop", "korean_est", "foreign_total"]]
        # 노출지수의 분모는 그 구의 전체 인구다: 내국인 + 등록외국인.
        # 주민등록(total_pop)은 내국인 명부라 그것만으로는 분모가 되지 않는다.
        pop_idx = pop_idx.assign(
            resident_total=pop_idx["korean_est"] + pop_idx["foreign_total"])
        ...
        for country, ct_sub in sg_ctry.groupby("country"):
            ...
            for (sido, sg), x_i in ct_sub.set_index(["sido", "sigungu"])["n"].items():
                ...
                if not tot_pop or not t_i or x_i == 0:
                    continue
                if X > 0:
                    isolation += (x_i / X) * (x_i / t_i)
                    interaction_kor += (x_i / X) * (pop_idx.loc[(sido, sg), "korean_est"] / t_i)
        ...
        if national_total_pop and kor_total is not None and kor_total > 0:
            ...
            # 전체 인구 = 내국인 + 외국인. national_total_pop 은 주민등록이라
            # 내국인만 센다. 각 구의 t_i 와 같은 셈법으로 맞춘다.
            T = sum(group_tot.values())
            E = 0.0
            ...
            if p and p > 0 and sido in lvl0:
                ...
                if denom_for > 0:
                    cont_shares = {k: round(v / denom_for * 100, 3)
                                   for k, v in sorted(cont.items(), key=lambda x: -x[1])}
                cont_full = dict(cont)
                cont_full["동아시아"] = cont_full.get("동아시아", 0) + max(p, 0)
                denom = sum(cont_full.values())
                if denom > 0:
                    cont_H = 0.0
                    for v in cont_full.values():
...
def export_json(stay_long, reg_long, out_path):
    ...
    data = {
        ...
        "source_ko": "법무부 출입국·외국인정책 통계연보 (2006–2024)",
        "source_en": "Korea Immigration Service Statistical Yearbook (2006–2024)",
        "visa_info": {
            code: {
                "code": code,
...
def export_region_age_json(region_df, age_df, out_dir):
    ...
    with open(region_path, "w", encoding="utf-8") as f:
        json.dump({
            ...
            "source_ko": "법무부 출입국·외국인정책 통계연보 · 시군구별 등록외국인 (2014~2024)",
            "source_en": "KIS Yearbook · Registered foreigners by sigungu (2014–2024)",
    ...
    with open(age_path, "w", encoding="utf-8") as f:
        ...
        json.dump({
            ...
            "age_groups": all_ages,
            "data": age_out,
            "source_ko": "법무부 출입국·외국인정책 통계연보 · 국적 및 연령별 체류외국인 (2014~2024)",
            "source_en": "KIS Yearbook · Foreign residents by nationality and age (2014–2024)",
        }, f, ensure_ascii=False, separators=(",", ":"))
...
# 접힌 체류자격 하위 코드를 남긴다 (기탁본 crosswalk_visa.csv 의 원천)
import csv as _csv
_vc = os.path.join(OUT_DATA, 'visa_code_collapse.csv')
with open(_vc, 'w', encoding='utf-8-sig', newline='') as _f:
    _w = _csv.writer(_f)
    _w.writerow(['source_code', 'visa_code'])
    for _k in sorted(VISA_COLLAPSED):
        _w.writerow([_k, VISA_COLLAPSED[_k]])
print('visa_code_collapse.csv saved: %d sub-codes folded into parents' % len(VISA_COLLAPSED))
