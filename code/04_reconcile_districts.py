...
def fix_subnational():
    ...
    # from national_language.json (covers 2006-2025, computed in build_national_
...
def recompute_from_reconciled():
    ...
    def recompute_summary():
        ...
        for y in idx["by_sigungu"]:
            ...
            cont_full = dict(cont)
            cont_full["동아시아"] = cont_full.get("동아시아", 0) + max((natpop or nf) - nf, 0)
...
def normalize_top19():
    ...
    for ystr in years_done:
        ...
        s["national_shannon_H_inclusive"] = round(shannon({**reduced, "_korean_residual": max((natpop or nf) - nf, 0)}), 3) if natpop else None
        s["n_nationalities"] = n
