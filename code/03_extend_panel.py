...
def add_lisa(adj):
    """Local Moran of the foreign share, per year, onto indices.json."""
    doc = json.load(open(IDX, encoding="utf-8"))
    d = doc["data"]

    for y in d["years"]:
        rows = d["by_sigungu"][str(y)]
        by_key = {key(r): r for r in rows}
        sk = [k for k in by_key if k in adj and by_key[k].get("foreign_share_pct") is not None]
        sk = [k for k in sk if any(n in set(sk) for n in adj.get(k, []))]
        nb = {k: [n for n in adj[k] if n in set(sk)] for k in sk}
        W = libpysal.weights.W(nb, silence_warnings=True)
        W.transform = "r"
        order = W.id_order
        vals = np.array([by_key[k]["foreign_share_pct"] for k in order])
        lm = esda.Moran_Local(vals, W, permutations=999, seed=42)
        sig = lm.p_sim < 0.05
        cat = {order[i]: (LAB[lm.q[i]] if sig[i] else "ns") for i in range(len(order))}
        for r in rows:
            r["lisa"] = cat.get(key(r), None)

    json.dump(doc, open(IDX, "w", encoding="utf-8"), ensure_ascii=False)
    latest = str(max(int(y) for y in d["years"]))
    print(f"{latest} LISA categories:",
          dict(Counter(r.get("lisa") for r in d["by_sigungu"][latest])))
    print("wrote", IDX)
...
def parse_sido_2006_2013():
    ...
    def continent(counts, pop):
        ...
        full = dict(reg); kor = max(pop - ftot, 0)
...
def build_visa_sigungu():
    ...
    2008-2025 into a single panel for the dashboard and the dataset release.
    ...
    three variants:
      ...
      * 2010-2011, 2014-2025: sido in col0 (block header), sigungu in col1,
