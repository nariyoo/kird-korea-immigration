...
def attach_breakdowns():
    ...
    def main():
        ...
        # ===== summary_national — full national summary at data/ top (was national_annual) =====
        ...
        if os.path.exists(na_src):
            ...
            def _settle(r):
                ...
                return pd.Series({"settlement_rate_pct": _pct(nat_ + ch, tot), "labor_dependence_pct": _pct(w, und),
                                  ...
                                  "settlement_type": st})
            na = pd.concat([na, na.apply(_settle, axis=1)], axis=1)
            ...
            out_na = os.path.join(DATA, "summary_national.csv")
            na.to_csv(out_na, index=False, encoding="utf-8-sig")
            for v, dd_ in lrn:
                rows.append(("summary_national.csv", v, dd_))
            print(f"summary_national: {len(na)} rows, {len(na.columns)} cols (+{len(lrn)})")
        ...
        # the deposit renames national_annual -> summary_national (now a full national summary at data/ top)
        base_dict["file"] = base_dict["file"].astype(str).str.replace("national_annual.csv", "summary_national.csv", regex=False)
...
def final_qc():
    ...
    files = ["summary_national","summary_by_sido","summary_by_sigungu","summary_by_eupmyeondong"]
