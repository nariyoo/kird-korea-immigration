...
The English district names are audited here first, because `sigungu_en` is copied
verbatim out of the boundary file and a wrong romanization would ship in the
deposit. That audit is also runnable on its own:

    python 02_code/08_export_dataset.py --check-names

which prints every district whose `name_eng` breaks a rule and exits 1 if any do.
"""
import csv
import io
import json
...
# and _load_en_lookups below). Nothing else in the pipeline checks that value, so
# a wrong romanization in the boundary file reaches the deposit unnoticed. In
# August 2026 eight of them were wrong, including 철원군 published as "Gongju-si"
# across 1,877 rows.
#
...
def check_geojson(path=GEO, verbose=True):
    ...
    for f in geo["features"]:
        ...
        m = re.match(r"^(.+시)(.+구)$", nm)
        pairs = list(zip(m.groups(), en.split())) if (m and len(en.split()) == 2) \
            else [(nm, en)]
        for a, b in pairs:
...
def export_dataset():
    ...
    def main():
        ...
        # 2a-2) v1.2.0 level-parallel companions. Every breakdown should exist at
        #       all three levels so no level has to be reconstructed from another.
        #       The sido table is the district table summed within the province, so
        #       it carries the allocated basis and its caveat (people the yearbook
        ...
        w("nationality_national.csv", add_en(nat_nat, ["country"]))
        visa_nat = (visa.groupby(["year", "population", "visa_code"], as_index=False)
                        .agg({"visa_label": "first", "n": "sum"})
                        .sort_values(["year", "population", "visa_code"]))
        w("visa_national.csv", visa_nat)

        # 2b) Foreign residents by sigungu x visa status x year (2017-2024 only;
        ...
        for y, arr in idx["by_sigungu"].items():
            for s in arr:
                rows.append({"year": int(y), "sido": s["sido"], "sigungu": s["sigungu"],
                             ...
                             # v1.2.0: the old n_nationalities was the size of the
                             # index base, not a count of nationalities. The two are
                             # now separate columns and the misleading name is gone.
                             "index_base_k": s.get("n_nationalities"),
                             "n_nationalities_observed": s.get("n_nationalities_observed")})
        ...
        if rows:
            nsa = pd.DataFrame(rows).sort_values("year")
            # v1.2.0: the MOIS settlement-composition block the sido and sigungu
            # summaries carry, summed to the national level (counts sum across
            # levels; a year is filled only when every province reports, so a
            # partial year stays blank rather than understating). The derived
            # percentages use the same formulas as 06_build_summaries.
            sb_p = os.path.join(OUTD, "summary_by_sido.csv")
            if os.path.exists(sb_p):
                ...
                def _pct(nu, de):
                    return (100 * nu / de).round(2)
                nsa["settlement_rate_pct"] = _pct(
                    nsa["naturalized"] + nsa["children"], nsa["broad_total"])
                nsa["labor_dependence_pct"] = _pct(nsa["workers"], nsa["non_naturalized"])
                nsa["marriage_dependence_pct"] = _pct(nsa["marriage_migrants"], nsa["non_naturalized"])
                nsa["study_dependence_pct"] = _pct(nsa["students"], nsa["non_naturalized"])
            w("national_summary_annual.csv", nsa)

        # 6d) Region-of-origin segregation
        rows = []
        for y, blk in idx.get("region_seg", {}).items():
            for region, g in blk.items():
                # v1.2.0: the values in this column are continents of origin, and
                # the old name region read like a geographic key. Renamed.
                rows.append({"year": int(y), "continent": region, "total": g.get("total"),
                             "dissimilarity_D": g.get("D"), "isolation": g.get("isolation")})
        if rows:
            w("region_segregation.csv", pd.DataFrame(rows).sort_values(["year", "continent"]))
            ...
            for key, langs in blk["by_sigungu"].items():
                ...
                for x in langs:
                    rows.append({"year": int(y), "scope": "sigungu", "sido": sido, "sigungu": sg,
                                 "language": x["language"], "count": x["count"]})
        # DRAFT ONLY. The released language_demand.csv is written by the
        # released-version writer further down (untrimmed block, integer counts,
        # sido scope); this draft exists to seed its English-label lookup. It
        # carries the _draft suffix so nobody edits a file that gets replaced —
        # the v1.2.0 sido rows were first added here and silently thrown away.
        w("language_demand_draft.csv", add_en(pd.DataFrame(rows).sort_values(["year", "scope", "sido", "sigungu"]), ["sido", "sigungu", "language"]))
    ...
    def write_dictionary():
        """BOOTSTRAP ONLY — skipped whenever the curated dictionary exists,
        which is always after the first run. The released data_dictionary.csv
        is REGENERATED by 09_finish_release.build_data_dictionary() from its
        curated bilingual spec; column changes must be made THERE, or the
        audit gate fails. Editing the tuples below changes nothing."""
        fields = [
            ("foreign_residents_by_visa.csv", "year", "integer", "Reference year (2006-2024)"),
            ...
            ("foreign_residents_by_sigungu_visa.csv", "visa_code", "string", "Visa/stay-status code (e.g., E9, F4); registered foreigners by district x visa, 2017-2024 (source table not published before 2017)"),
            ...
            ("indices_by_sigungu.csv", "evenness", "float", "Pielou evenness = Shannon H / ln(index_base_k); 0-1, higher = nationalities more evenly balanced"),
            ("indices_by_sigungu.csv", "index_base_k", "integer", "Number of categories the diversity indices are computed over: the top 19 nationalities of that year plus one residual bin. It is 20 wherever at least 20 nationalities are present, so it is a property of the index basis, not of the district. Named n_nationalities through v1.1.0."),
            ("indices_by_sigungu.csv", "n_nationalities_observed", "integer", "Distinct nationalities the source actually lists for that unit and year, residual bin excluded. Capped at 19 for 2008-2013, when the yearbook publishes only the top 19 plus a residual at the district level; full detail from 2014."),
            ...
            ("indices_by_sido.csv", "(all index columns)", "mixed", "Same index set as indices_by_sigungu (foreign_share_pct, shannon_H, shannon_H_inclusive, continent_H, HHI, evenness, index_base_k, n_nationalities_observed), computed with the province treated as one unit, 2006-2024. Indices are recomputed at this level, never averaged from the districts."),
...
## Sources
- Ministry of Justice (MOJ) Korea Immigration Service Statistical Yearbook (출입국·외국인정책 통계연보), 2006-2024 — foreign residents by nationality x visa x district. Official yearbooks: https://www.immigration.go.kr/immigration/1570/subview.do
- Ministry of the Interior and Safety (MOIS) Resident Registration Population (denominator)

## Files (data/)
| File | Unit | Years |
|---|---|---|
| foreign_residents_by_visa.csv | population x nationality x visa x year | 2006-2024 |
| foreign_residents_by_sigungu.csv | sido x sigungu x nationality x year | 2009-2024 |
| foreign_residents_by_sigungu_visa.csv | sido x sigungu x visa status x year | 2008-2024 |
| foreign_residents_by_age_sex.csv | nationality x age x sex x year | 2009-2024 |
| resident_population_by_sigungu.csv | sido x sigungu x year (MOIS) | 2008-2024 |
| indices_by_sigungu.csv | sido x sigungu x year | 2009-2024 |
| indices_by_sido.csv | sido x year | 2006-2024 |
| segregation_by_nationality.csv | nationality x year (D, isolation, interaction) | 2014-2024 |
| region_segregation.csv | region of origin x year (D, isolation) | 2014-2024 |
| national_summary_annual.csv | year (diversity + Theil + Moran's I series) | 2009-2024 |
| ethnic_enclaves.csv | year x sigungu x nationality | 2009-2024 |
| language_demand.csv | year x scope x language | 2009-2024 |
...
reconcile to within ~0.2% every year 2006-2024 (e.g. 2007 -0.03%, 2024 exact);
...
def export_language_demand():
    ...
    DRAFT = os.path.join(os.path.dirname(OUT), "language_demand_draft.csv")
    seed = DRAFT if os.path.exists(DRAFT) else OUT
...
if __name__ == "__main__":
    if "--check-names" in sys.argv:
        # the boundary-file audit on its own, without rebuilding anything
